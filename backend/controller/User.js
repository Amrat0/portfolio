// controller ka under hum model jo banata ha woh use karaga usko import karga
...
export const updateUser= async(req,res)=>{
  try{
       ...
       if (about) {
        ...
        if(about.avator){
          await cloudinary.v2.uploader.destroy(user.about.avator.public_id)
          const myCloud=await cloudinary.v2.uploader.upload(about.avator,{
            folder: 'portfolio'
          });
          user.about.avator={
            public_id: myCloud.public_id,
...
export const contact=async(req,res)=>{
       try{
           ...
           const userMessage=  `Hey, I am ${name}. My email ${email}. My message is ${message}.`
...
export const addTimeline = async(req,res)=>{
  try{
   ...
   const user= await User.findById(req.user._id)
  //  push nhi karaga hum q ka push last main add karta ha hum unshift karga q  ka uper laka ahyaga
...
export const deleteProject=async(req,res)=>{
  try{
         const {id}=req.params;
         const user=await User.findById(req.user._id)
         const proj=user.projects.find((item)=>item._id==id )
         await cloudinary.v2.uploader.destroy(proj.image.public_id)
